    ...
    function post_initialize() {
        ...
        function onMessage(topic, payload) {
            ...
            node = paths[top] = (paths.hasOwnProperty(top)) ? paths[top] :
                                server.engine.addressSpace.getOwnNamespace().addFolder("ObjectsFolder", { browseName: top});
            ...
            for (sub in path) {
                ...
                if (topic !== top) {
                    node = paths[top] = (paths.hasOwnProperty(top)) ? paths[top] :
                                        server.engine.addressSpace.getOwnNamespace().addFolder(node, { browseName: path[sub]});
                } else {
                    debug("Creating folders for new topic: " + topic);
                    persist[topic] = handler(payload).value;
                    flagged[topic] = false;
                    try {
                        nodes[topic] = server.engine.addressSpace.getOwnNamespace().addVariable({
                            componentOf: node,
                            browseName: path[sub],
                            path: topic,
                            nodeId: "s=" + topic,
